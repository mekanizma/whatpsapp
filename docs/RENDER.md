...
**Önemli:** Render Dashboard → servisiniz → **Disks** sekmesinde `whatsapp-sessions` diskinin bağlı olduğunu doğrulayın. Disk yoksa her deploy/restart sonrası QR gerekir.

**Uyku modu:** Starter planda 15 dk HTTP trafiği yoksa servis uyur. `render.yaml` içindeki `whatsapp-ai-keepalive` cron job her 10 dakikada `/health` ping atar. Cron URL'sini kendi domain'inizle güncelleyin.
...
| Kontrol | Beklenen |
|---------|----------|
| `SESSIONS_DIR` | `/opt/render/project/src/data/sessions` |
| Disk mount | `/opt/render/project/src/data` |
| Plan | Starter veya üzeri (disk için) |
| Keep-alive cron | Aktif ve doğru URL |
...
| WhatsApp kopuyor | Disk mount + `SESSIONS_DIR` eşleşmeli; keep-alive cron aktif mi |
| Her restart'ta QR | Kalıcı disk bağlı değil — Dashboard → Disks |
| Kopunca QR istiyor | WhatsApp cihazdan oturumu kapatmış olabilir; logda `loggedOut` |
